...
def _handle_launchagent_command(args: argparse.Namespace) -> int:
    ...
    if sub == "status":
        ...
        env_path = (plist.get("EnvironmentVariables") or {}).get("PATH", "")
        ...
        if env_path:
            n = len([p for p in env_path.split(":") if p])
            env_path_s = f"{c['good']('set')}  {c['dim'](f'({n} entries)')}"
        else:
            env_path_s = c["warn"]("not set")
